...
control '2.2.28_L2_Ensure_Log_on_as_a_batch_job_is_set_to_Administrators' do
  ...
  tag cce: 'CCE-33432-6'
  allowed_principals = ['S-1-5-32-544']
  describe security_policy.SeBatchLogonRight - allowed_principals do
    it { should be_empty }
  end
end
...
control '2.3.14.1_L2_Ensure_System_cryptography_Force_strong_key_protection_for_user_keys_stored_on_the_computer_is_set_to_User_is_prompted_when_the_key_is_first_used_or_higher' do
  ...
  desc  "
    ...
    The recommended state for this setting is: User is prompted when the key is first used. Configuring this setting to User must enter a password each time they use a key also conforms with the benchmark.
...
control '18.3.4_L2_Ensure_MSS_DisableSavePassword_Prevent_the_dial-up_password_from_being_saved_is_set_to_Enabled' do
  ...
  desc  "
    ...
    Rationale: An attacker who steals a mobile user's computer could automatically connect to the organization's network if the **Save This Password** check box is selected for the dial-up networking entry used to connect to your organization's network.
  "
  impact 1.0
...
control '18.9.35.1_L2_Ensure_Turn_off_location_is_set_to_Enabled' do
  ...
  desc  "
    ...
    Rationale: This setting affects the location feature (e.g. GPS or other location tracking). From a security perspective, it#x2019;s not a good idea to reveal your location to software in most cases, but there are legitimate uses, such as mapping software.
  "
  impact 1.0
  tag cce: 'CCE-33743-6'
...
control '18.9.38.2_L2_Ensure_Dont_allow_SmartScreen_Filter_warning_overrides_is_set_to_Enabled' do
  ...
  desc  "
    ...
    Rationale: SmartScreen will warn a user is a website or file is potentially malicious. Enabling this setting prevents these warnings from being bypassed.
  "
  impact 1.0
...
control '18.9.38.8_L2_Ensure_Turn_off_Pop-up_Blocker_is_set_to_Enabled' do
  ...
  desc  "
    ...
    Rationale: The Pop-up Blocker can serve an important purpose by blocking malicious popups and helping prevent the machine from being compromised.
  "
  impact 1.0
  describe registry_key('HKEY_LOCAL_MACHINE\\SOFTWARE\\Policies\\Microsoft\\MicrosoftEdge\\Main') do
...
control '19.6.5.1.1_L2_Ensure_Turn_off_Help_Experience_Improvement_Program_is_set_to_Enabled' do
  ...
  impact 1.0
  tag cce: 'CCE-33349-2'
  registry_key({ hive: 'HKEY_USERS' }).children(/^S-1-5-21-[0-9]+-[0-9]+-[0-9]+-[0-9]{3,}\\[Ss][Oo][Ff][Tt][Ww][Aa][Rr][Ee]\\Policies\\Microsoft\\Assistance\\Client\\1.0/).each do |entry|
    describe registry_key(entry) do
...
control '19.7.41.2.1_L2_Ensure_Prevent_Codec_Download_is_set_to_Enabled' do
  ...
  impact 1.0
  tag cce: 'CCE-33793-1'
  registry_key({ hive: 'HKEY_USERS' }).children(/^S-1-5-21-[0-9]+-[0-9]+-[0-9]+-[0-9]{3,}\\[Ss][Oo][Ff][Tt][Ww][Aa][Rr][Ee]\\Policies\\Microsoft\\WindowsMediaPlayer/).each do |entry|
    describe registry_key(entry) do
